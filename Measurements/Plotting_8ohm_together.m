...
%%
amplL=load('amplitude_137L_parallell_8ohm.txt');
freqsL=amplL(:,1);
amplL=amplL(:,2);
ampl_dBL=20*log10(amplL);
phaseL=load('phase_137L_parallell_8ohm.txt');
phaseL=phaseL(:,2);

%%
amplR=load('amplitude_165R_parallell_8ohm.txt');
freqsR=amplR(:,1);
amplR=amplR(:,2);
ampl_dBR=20*log10(amplR);
phaseR=load('phase_165R_parallell_8ohm.txt');
phaseR=phaseR(:,2);
...
legend({'Amplitud del canal izquierdo', 'Amplitud del canal derecho', 'Fase del canal izquierdo', 'Fase del canal derecho'}, 'Fontsize', 20, 'Position', [0.686111116295473 0.635340952510114 0.200520828148971 0.14278296580072])
